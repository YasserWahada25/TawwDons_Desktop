    ...
    private Callback<TableColumn<DemandeDons, DemandeDons>, TableCell<DemandeDons, DemandeDons>> getActionCellFactory() {
        return column -> new TableCell<>() {
...

            {
                ...
                btnValider.setOnAction(e -> { DemandeDons demande = getTableView().getItems().get(getIndex());
                    try { FXMLLoader loader = new FXMLLoader(getClass().getResource("/Dons/SignaturePopup.fxml"));
                        Parent root = loader.load(); controllers.dons.SignaturePopupController controller = loader.getController();
                        Stage popup = new Stage(); popup.setScene(new Scene(root));
                        popup.setTitle("Signature du Donneur");
                        popup.showAndWait();
                        File signatureFile = controller.getSignatureFile();
                        if (signatureFile != null && signatureFile.exists()) { boolean validated = donsService.validerDemande(demande.getId());
                            if (validated) { boolean success = RecuPDFGenerator.generateRecu(demande, signatureFile.getAbsolutePath());
                                showAlert(success, "✅ Demande validée et reçu généré !", "❌ PDF non généré !");
                                refreshTable(); } else { showAlert(false, "", "❌ Échec validation de la demande !"); } } }
                    catch (IOException ex) { ex.printStackTrace(); showAlert(false, "", "❌ Erreur lors de l'ouverture de la signature !"); } });

                btnTelecharger.setOnAction(e -> { DemandeDons demande = getTableView().getItems().get(getIndex()); boolean success = RecuPDFGenerator.generateRecu(demande, null); showAlert(success, "✅ Reçu téléchargé avec succès !", "❌ Échec du téléchargement !"); });
            }
